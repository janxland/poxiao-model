            ...
            <div v-for="item, index in i.questionVoList" :key="'exams'+index" class="mt-4">
                ...
                <div v-if="i.type == 1">
                    <t-radio-group v-model="item.ans" allow-uncheck :default-value="item.ans" @change="onChange" class="!block">
                        <t-radio-button :value="answerIndex" v-for="answer,answerIndex in item.content" :key="'answer'+answerIndex"  :disabled="disabled"
                ...
                <div v-else-if="[2,3,5,6,7].includes(Number(i.type)) ">
                    <div class="my-4">
                        答案：
                        <t-textarea placeholder="请输入你的答案" :autosize="{ minRows: 3}" v-model="item.ans" :defaultValue="item.ans" :disabled="disabled"/>
                    </div>
                    <div class="my-4">
                        你的答案：
                        <t-textarea placeholder="请输入你的答案" :autosize="{ minRows: 3}" v-model="item.correct" :defaultValue="item.correct" :disabled="disabled"/>
                    </div>
                </div>
                    ...
                    <div>
                        <t-radio-group v-model="item.ans" allow-uncheck :default-value="item.ans" @change="onChange" class="!block">
                            <t-radio-button value="0" :disabled="disabled" class="!border-0 !h-10 !w-8 mr-4">
                                <div class="answer" :class="checkAnswer(item,0)"><icon name="check"></icon></div>
                            </t-radio-button>
                            <t-radio-button value="1" :disabled="disabled" class="!border-0 !h-10 !w-8 mr-4">
                                <div class="answer" :class="checkAnswer(item,1)"><icon name="close"></icon></div> 
                            </t-radio-button>
                        </t-radio-group>
                    </div>
                </div>
...
const questionList_Default = ref([
        {
          name: '选择题',
          type: 1,
          questionVoList: [
